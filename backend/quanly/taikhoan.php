...
$query_user = mysqli_query($conn,"SELECT * FROM taikhoan");
?>
		...
		<div class="container-fluid">
			<div class="card card-info">
				<div class="card-header">
					...
					<a href="createTK.php" class="btn btn-warning btn-xs float-right" style="color:black;"><i class="fas fa-plus-circle"></i> Thêm mới</a>
				</div>
				<div class="card-body">
					<table class="table table-bordered table-striped projects" id="TableTaiKhoan" width="100%">
						<thead>
							<tr>
								<th>STT</th>
								<th>Mã tài khoản</th>
						...
						<tbody>
							...
							<tr>
								<td><?php echo $i;?></td>
								<td><?php echo $row_user['MaTK'];?></td>
								<td><?php echo $row_user['TenDangNhap'];?></td>
								<td><?php echo $row_user['Email'];?></td>
								<td><?php echo $row_user['VaiTro'];?></td>
								<td><?php echo isset($row_user['HoTen']) ? $row_user['HoTen'] : '';?></td>
								<td><?php echo isset($row_user['SoDienThoai']) ? $row_user['SoDienThoai'] : '';?></td>
								<td><?php echo isset($row_user['DiaChi']) ? $row_user['DiaChi'] : '';?></td>
								<td><a href="xoaTK.php?MaTK=<?php echo $row_user['MaTK'];?>" onclick="return confirm('Bạn có muốn xóa tài khoản này?')" class="btn btn-danger btn-sm float-right deletetable"><i class="fas fa-trash"></i>Xoá</a>
								<a href="editTK.php?MaTK=<?php echo $row_user['MaTK'];?>" class="btn btn-info btn-sm float-right edittable"><i class="fas fa-pencil-alt"></i>Sửa</a></td>
							</tr>
							<?php $i++; } ?>
						</tbody>
						<tfoot>
...
$(function () {
    ...
    }).buttons().container().appendTo('#TableTaiKhoan_wrapper .col-md-6:eq(0)');
  });
</script>
