...
$query_book = mysqli_query($conn,"SELECT * FROM sach");
?>
		...
		<div class="container-fluid">
			<div class="card card-info">
				<div class="card-header">
					...
					<a href="createSach.php" class="btn btn-warning btn-xs float-right" style="color:black;"><i class="fas fa-plus-circle"></i> Thêm mới</a>
				</div>
				<div class="card-body">
					<table class="table table-bordered table-striped projects" id="TableSach" width="100%">
						<thead>
							<tr>
								<th>STT</th>
								<th>Tên sách</th>
						...
						<tbody>
							<?php
							$i = 1; 
							while($row_book = mysqli_fetch_assoc($query_book)){ ?>
							<tr>
								<td><?php echo $i;?></td>
								<td><?php echo $row_book['TenSach'];?></td>
                                <td><?php $MaDM = $row_book['MaDM'];
										$row_cate = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM danhmuc WHERE MaDM = '$MaDM'"));
										$parent_MaDM = $row_cate['parent_MaDM'];
										if($parent_MaDM == 0) echo $row_cate['TenDM']; else{  
								...
								<td><?php echo  number_format($row_book['GiaGoc'],0,'.','.');?>đ</td>
								<td><?php echo $row_book['SoLuongCo'];?></td>
								<td><img style="height:120px;" src="../../upload/sach/<?php echo $row_book['AnhChup'];?>" alt="<?php echo $row_book['TenSach'];?>"></td>
                                <td><?php echo $row_book['TacGia'];?></td>
								<td><?php echo isset($row_book['NgayPhatHanh']) ? date('d-m-Y',strtotime($row_book['NgayPhatHanh'])) : '';?></td>
								<td><?php echo isset($row_book['DichGia']) ? $row_book['DichGia'] : '';?></td>
                                <td><?php echo isset($row_book['NXB']) ? $row_book['NXB'] : '';?></td>
								<td><a href="xoaSach.php?MaSach=<?php echo $row_book['MaSach'];?>" onclick="return confirm('Bạn có muốn xóa cuốn sách này?')" class="btn btn-danger btn-sm float-right delete"><i class="fas fa-trash"></i>Xoá</a>
								<a href="editSach.php?MaSach=<?php echo $row_book['MaSach'];?>" class="btn btn-info btn-sm float-right edit"><i class="fas fa-pencil-alt"></i>Sửa</a></td>
							</tr>
							<?php $i++; } ?>
						</tbody>
						<tfoot>
...
<script>
  $(function () {
    ...
    }).buttons().container().appendTo('#TableSach_wrapper .col-md-6:eq(0)');
  });
</script>
